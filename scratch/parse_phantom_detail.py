...
with open("scratch/phantom_detail.html", "r", encoding="utf-8") as f:
    html = f.read()
...
for tag in soup.find_all(['span', 'p', 'div', 'td', 'a']):
    ...
    if len(text) > 15:
        ...
        if txt_clean not in found:
            if any(w in txt_clean for w in ["May 31", "2026", "Janet Gardner", "tickets", "performing", "venue", "theatre", "opera", "St."]):
                found.add(txt_clean)
                print("  -", txt_clean[:200])
